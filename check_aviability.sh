...
list=$1
yc_zone_name=$2
LOGFILE="./logs/yc-dns_${yc_zone_name}.log"
readarray -t server_list < ./include/$list
...
if (( attempts_fail > 0 )) 
 then 
	  echo "`date "+%Y-%m-%d %H:%M:%S"` host ${host} is failed, trying ${attempts_fail} times again...." >> $LOGFILE
 else
	  host_disabled=$(grep /var/tmp/${list}_tmp/${host} -e 'disabled' | wc -l)
	  if [[ $host_disabled -eq "1" ]]
	  then 
		  echo "`date "+%Y-%m-%d %H:%M:%S"` host ${host} already disabled, do nothing" > /dev/null
	  else
		  echo "`date "+%Y-%m-%d %H:%M:%S"` Host ${host} check state failed, disabling in zone ${list} on Yandex Cloud DNS" >> $LOGFILE
		  echo "`date "+%Y-%m-%d %H:%M:%S"` attempts ends, disable host ${host} in zone ${list} on Yandex Cloud DNS" >> $LOGFILE
		  ...
		  then 
		    ...
		    echo "`date "+%Y-%m-%d %H:%M:%S"` all hosts disabled (((!!!))), can't disable last one host ${host} in zone ${list} on Yandex Cloud DNS" >> $LOGFILE 
...
then 
echo "`date "+%Y-%m-%d %H:%M:%S"` host ${host} avilable" > /dev/null
else 
	attempts_success=$((attempts_success-1))
	if (( attempts_success > 0 )) 
	 then 
		  echo "`date "+%Y-%m-%d %H:%M:%S"` host ${host} is up, trying ${attempts_success} times again...." >> $LOGFILE
	 else
			  echo "`date "+%Y-%m-%d %H:%M:%S"` Host ${host} check state success, enabling in zone ${list} on Yandex Cloud DNS" >> $LOGFILE
			  echo "`date "+%Y-%m-%d %H:%M:%S"` attempts ends, enable host ${host} in zone ${list} on Yandex Cloud DNS" >> $LOGFILE
			  echo "`date "+%Y-%m-%d %H:%M:%S"` host ${host} enabled" > /var/tmp/${list}_tmp/${host}
...
########### main code ##############
echo "-----------------------------------------------------------" >> $LOGFILE
echo "`date "+%Y-%m-%d %H:%M:%S"` Running script for zone ${list}" >> $LOGFILE
echo "-----------------------------------------------------------" >> $LOGFILE
